...
import type { PluginModule, PluginScope } from '@mini-canvas/kernel'
...
describe('插件 ctx 能力段收口（ctx.nodes/theme/commands/slots）', () => {
  ...
  /**
   * 类型级能力必须**逐字段**从 ctx.nodes.register 透传到 nodeStore。
   *
   * 这条测试的由来是一个真实 bug：给图片节点加 frameless（不要卡片边框）时，
   * registerNodeType 那条路加了、capabilities 这条路的透传清单漏了 ——
