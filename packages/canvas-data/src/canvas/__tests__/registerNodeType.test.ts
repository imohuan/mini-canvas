...
describe('registerNodeType（插件"一次自描述"节点注册接缝）', () => {
  ...
  /**
   * 类型级「不要卡片边框」能力（frameless）。
   *
   * 背景：卡片边框来自共享外壳（BaseNode），本意是让节点从画布背景上浮起来。
   * 但对**内容铺满整张卡**的类型（图片）来说，边框只会变成一圈多余的缝
   * ——用户实测到的"图片边上还有一像素边距"就是它。
   * 类型自己最清楚该不该有边框，所以在类型声明里表达（与 resizable/icon 同一层），
   * 渲染层读它，而不是让外壳去猜节点类型。
   */
  it('声明 frameless 的类型：原样透传进 nodeStore（渲染层据此不画卡片边框）', async () => {
    const { nodeStore } = await bootWithSetup((ctx) => {
      registerNodeType(ctx, { type: 'edge-to-edge', label: '贴满', defaultSize: { w: 10, h: 10 }, frameless: true })
      registerNodeType(ctx, { type: 'normal', label: '普通', defaultSize: { w: 10, h: 10 } })
    })
    expect(nodeStore.types.get('edge-to-edge')?.frameless).toBe(true)
    expect(nodeStore.types.get('normal')?.frameless).toBeUndefined()
  })
})
