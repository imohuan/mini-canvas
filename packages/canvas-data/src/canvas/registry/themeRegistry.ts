/**
 * themeRegistry —— 画布"外观/UI"注册表（画布领域，纯逻辑零 Vue，Node 可单测）。
 *
 * 归属：画布能力层（src/canvas）。原先住在 core/registry，因内容全是画布概念（节点壳/连线/背景槽），
 * 与"插件框架不认识画布"的边界冲突，故迁出；旧路径保留一行转发垫片。
 *
...
import type { SlotEntry } from '@mini-canvas/kernel'
