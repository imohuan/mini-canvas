 ...
 *
 * 归内核契约层（src/contracts，同 nodeRegistryKey.ts），不归 demo-web：
...
/**
 * 宿主句柄（形状见 src/host/createMiniCanvasHost.ts 的 CanvasHostHandle）的响应式引用。
 * 内核异步 boot，provide 的是"宿主引用"（开始时空、boot 完成后填充），
 * content 组件在交互时（此时 boot 必已结束）读取 .value。
 */
// 用 any 兜底避免渲染契约层反向 import 造成循环；具体类型由宿主在 provide 端标注。
