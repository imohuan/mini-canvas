...
function handleVideoDownload(ctx: CommandContext) {
  const videoUrl = (ctx.node?.data as any)?.videoUrl as string | undefined
  if (!videoUrl) return
  const a = document.createElement('a')
  a.href = videoUrl
  a.download = ((ctx.node?.data as any)?.videoName as string | undefined) || 'video.mp4'
  a.click()
}
...
export const VideoNodePlugin: CanvasPlugin = {
  ...
  install(context: PluginContext) {
    ...
    context.toolbars.register('node:video', { id: 'video.download', source: 'node:video', commandId: 'video.download', position: 'bottom', title: '下载', icon: downloadSvg, nodeTypes: ['video'], order: 30 })
