...
import { keyCapLabel } from './keyLabels'
...
/** 显示片段：把原串拆成段并逐段译成键帽（'mod+z' → ['Ctrl'|'⌘', 'Z']） */
const displayParts = computed(() =>
  splitShortcutKeys(props.keys).map((p) => keyCapLabel(p)),
)
...
<template>
  <span :class="[wrapperClass ?? 'shortcut-keys', size === 'sm' ? 'is-sm' : '']">
    <template v-if="displayParts.length > 0">
      <template v-for="(part, pIdx) in displayParts" :key="uid(pIdx)">
        <span class="kbd-chip font-mono">{{ part }}</span>
        <span v-if="pIdx < displayParts.length - 1" class="kbd-plus">+</span>
      </template>
    </template>
