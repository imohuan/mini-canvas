...
import { computeCompactArrange, type CompactRect } from '../compactArrange'
...
describe('computeCompactArrange（老版核心算法搬迁）', () => {
  it('少于等于 1 个节点返回空 Map', () => {
    expect(computeCompactArrange([], 'ArrowLeft', GAP).size).toBe(0)
    expect(computeCompactArrange([R('a', 0, 0)], 'ArrowLeft', GAP).size).toBe(0)
  })

  it('ArrowLeft：重叠节点向左收拢并保持 gap，首个保持', () => {
    // a(0,0,100) 在最左保持；b 与 a y 重叠 → 收到 a 右 + gap
    const nodes = [R('a', 0, 0), R('b', 300, 0)]
    const out = computeCompactArrange(nodes, 'ArrowLeft', GAP)
  ...
  it('ArrowRight：重叠节点向右收拢，右边缘对齐的最右节点保持', () => {
    ...
    const out = computeCompactArrange(nodes, 'ArrowRight', GAP)
  ...
  it('ArrowUp：纵向收拢，最上节点保持，下方重叠节点贴上来保持 gap', () => {
    ...
    const out = computeCompactArrange(nodes, 'ArrowUp', GAP)
  ...
  it('ArrowDown：纵向收拢，最下节点保持，上方节点贴下来保持 gap', () => {
    ...
    const out = computeCompactArrange(nodes, 'ArrowDown', GAP)
