    ...
    if (sourcePosition !== targetPosition) {
      ...
      const flip =
        (sourceDir[dirAccessor] === 1 &&
          ((!isSameDir && sourceGt) || (isSameDir && sourceLt))) ||
        (sourceDir[dirAccessor] !== 1 &&
          ((!isSameDir && sourceLt) || (isSameDir && sourceGt)));
      if (flip) {
...
export function buildEdgePath(
  ...
  const app: Required<EdgeAppearance> = {
    ...DEFAULT_APPEARANCE,
    ...(appearance || {}),
  };
  switch (edgeType) {
