...
/**
 * 在命令列表里找出这次按键该执行的命令（渲染层 keydown 分发用）。无命中返回 undefined。
 *
 * 两条规则（用户要求："选中节点之后支持这些快捷键"，而 f/r 这类键画布上也已经注册了 → 必须有优先级）：
 *
 * 1. **when 不满足 = 让位**：该命令此刻不适用，就交给同键的其它命令，而不是"命中即吃掉、
 *    execute 时被 when 拦成空操作"——后者会让节点命令把全局快捷键永久挤没（没有节点时按 f 就什么也不发生）。
 *    没传 executeCtx 时不评估 when（老调用方行为不变）。
 * 2. **order 小的赢**：同键多个命令都可用时按 order 定优先级（越小越优先，与命令在
 *    菜单/工具栏里的排序语义一致）；order 相同才回落到注册先后，保证既有分发顺序不变。
 *
 * @param commands    候选命令（注册表顺序）
 * @param e           键盘事件
 * @param executeCtx  可选：执行上下文（通常是内核 ctx）。传了才评估 when。
 */
export function findCommandByKeys<T extends { id: string; keys?: string[] }>(
  ...
  for (const cmd of commands) {
    ...
    if (!commandMatchesKeys(cmd.keys, e)) continue
    // when 不满足 → 让位（不是吃掉）。不传 executeCtx 时跳过评估，保持老行为。
    const when = (cmd as { when?: (ctx: unknown) => boolean }).when
    if (when && executeCtx !== undefined && !when(executeCtx)) continue
    // order 小的赢；没写 order 视为最后（与 ShortcutHelpItem 的默认 100 无关：这里只看相对大小）
    const order = (cmd as { order?: number }).order
    const rank = typeof order === 'number' && Number.isFinite(order) ? order : Number.POSITIVE_INFINITY
    if (!winner || rank < winnerOrder) {
      winner = cmd
      winnerOrder = rank
    }
  }
