...
> **执行状态（2026-09）**：
> - 阶段二（默认渲染器收编进 default-theme）**已完成并浏览器验证**。commit `16fd206` + `804ae29`。
> - 阶段一（连线能力 props 封装 EdgeSlot）**按用户澄清暂缓**：path 仍由边自算，选中等"状态传给边"属后期方向，本轮只理解不实现。
> - 三个分叉拍板：①A(无兜底/主题即默认)、②(延后，见上)、③A(内容段仍 inject 分发)。
